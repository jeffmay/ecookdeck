...
describe("ProfileSettingsPage", () => {
  it("renders current name in the input", () => {
    render(<ProfileSettingsPage currentName="Alice" onSave={vi.fn()} onCancel={vi.fn()} />);
  ...
  it("calls onSave with trimmed name when Save is clicked", async () => {
    const onSave = vi.fn();
    render(<ProfileSettingsPage currentName="Alice" onSave={onSave} onCancel={vi.fn()} />);
    const input = screen.getByLabelText("Your name");
    await userEvent.clear(input);
    await userEvent.type(input, "  Bob  ");
  ...
  it("calls onCancel when Cancel is clicked", async () => {
    ...
    render(<ProfileSettingsPage currentName="Alice" onSave={vi.fn()} onCancel={onCancel} />);
